<div class="comment-item relative p-4 bg-white dark:bg-gray-800 rounded-lg shadow mb-3 border border-gray-200 dark:border-gray-700 opacity-0 translate-y-3 transition-all duration-300" id="comment-<?= esc($commentId ?? 'temp') ?>">
...
<script>
(() => {
    const newComment = document.querySelector('#comment-<?= esc($commentId ?? 'temp') ?>');
    if (newComment) {
        setTimeout(() => {
